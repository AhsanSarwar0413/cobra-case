...
export default async function DashboardPage() {
    ...
    const orders = await db.order.findMany({
        where: {
            isPaid: true,
            createdAt: {
                gte: new Date(new Date().setDate(new Date().getDate() - 7)), //gte = >= greater than or equal
            },
        },
        orderBy: {
            createdAt: 'desc'
    ...
    const lastMonthSum = await db.order.aggregate({
        where: {
            isPaid: true,
            createdAt: {
                gte: new Date(new Date().setDate(new Date().getDate() - 30)), //gte = >= greater than or equal
            },
        },
        _sum: {
            amount: true,
                        ...
                        <TableBody>
                            {orders.map((order) => (
                                <TableRow key={order.id} className="bg-accent">
                                    <TableCell>
                                        {/* we use question mark for the field that we have made optional in prisma ORM */}
